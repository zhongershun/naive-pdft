	...
	for _,peer :=range s.peers {
		...
		go func() {
			...
			if err := p.rpcCli.Call("Server.CommitRpc", args, reply); err != nil {
				zlog.Warn("Server.CommitRpc %d error %v", p.id, err)
			}
		}()
	}
...
func (s *Server) verifyBallotPrepare(cert *LogCert) {
	...
	for _, args := range argsQ {
		...
		// 验证PrepareMsg签名信息
		if ok := Verify(Digest(msg), args.Sign, s.peers[args.Msg.PeerId].pubkey); !ok {
...
func (s *Server) verifyBallotCommit(cert *LogCert) {
	...
	for _, args := range argsQ {
		...
		// 验证摘要是否相同
		if !SliceEqual(cert.digest, msg.Digest) {
			zlog.Warn("verify ballot, CommitMsg error, req.digest != msg.Digest")
			continue
		}
